...
```{r}
#install.packages("rvest") #install revest to read html files or webpages
library(rvest)
htmlLink <- "https://money.cnn.com/data/us_markets/"
webpage <- read_html(htmlLink)
```

2. Get the session details (status, type, size) of the above mentioned URL.
```{r}
#install.packages("rvest") #install rvest to get the session details of CNN Money webpage
#library(rvest)
html_session(htmlLink)
```

3. Extract all of the sector names from the “Stock Sectors” table (bottom left of the web page.)
```{r, eval=FALSE}
#select only the names of the table "Stock Sectors" by extracting from the first element
html_table(html_nodes(webpage, "table"))[[2]][[1]]
```

4. Extract all of the “3 Month % Change” values from the “Stock Sectors” table.
```{r}
#select all the values from the "Stock Sectors" table by extracting from the second element
html_table(html_nodes(webpage, "table"))[[2]][[2]]
```

5. Extract the table “What’s Moving” (top middle of the web page) into a data-frame.
```{r}
#extract the first table by selecting the first element
...
```{r}
# save all url links into table_link where css = td class wsod_symbol
table_links <- webpage %>% html_nodes(css = "td .wsod_symbol") %>% html_attr("href")

# Paste links to see all links from What's Moving table
paste("https://money.cnn.com", table_links, sep = "")
```

7. Extract the titles under the “Latest News” section (bottom middle of the web page.)
```{r}
#use html_text to extract titles where css = .HeadlineList a
...
```{r}
# use html_attrs to get the attributes of What's Moving table (remember css = wsod_disclaimer for this)
html_attrs(html_nodes(webpage, css = ".wsod_disclaimer > span"))
```
